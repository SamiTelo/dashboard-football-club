...
// Type minimal pour le token reçu côté client
...
export function GoogleLoginButton({
  ...
  const login = useGoogleLogin({
    onSuccess: (tokenResponse: unknown) => {
      
      const data = tokenResponse as GoogleTokenResponse;
      ...
      setLoading(true);
      googleLogin({ idToken: data.credential })
        .catch((err) => console.error("Erreur Google Login:", err))
        .finally(() => setLoading(false));
    },
    onError: () => {
      console.error("Google Login échoué");
    },
  });
